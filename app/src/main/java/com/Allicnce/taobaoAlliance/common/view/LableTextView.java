package com.Allicnce.taobaoAlliance.common.view;
 ...
 * @author HaoZhang
 */
public class LableTextView extends View {
    ...
    public LableTextView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        super(context, attrs, defStyleAttr, defStyleRes);
        init(attrs);
    }

    public void init(AttributeSet attrs) {
        TypedArray array = getContext().obtainStyledAttributes(attrs, R.styleable.LableTextView);

        mTextSize = array.getDimension(R.styleable.LableTextView_slantedTextSize, mTextSize);
        mTextColor = array.getColor(R.styleable.LableTextView_slantedTextColor, mTextColor);
        mSlantedLength = array.getDimension(R.styleable.LableTextView_slantedLength, mSlantedLength);
        mSlantedBackgroundColor = array.getColor(R.styleable.LableTextView_slantedBackgroundColor, mSlantedBackgroundColor);

        if (array.hasValue(R.styleable.LableTextView_slantedText)) {
            mSlantedText = array.getString(R.styleable.LableTextView_slantedText);
        }

        if (array.hasValue(R.styleable.LableTextView_slantedMode)) {
            mMode = array.getInt(R.styleable.LableTextView_slantedMode, 0);
        }
        array.recycle();
    ...
    private void drawBackground(Canvas canvas) {
        ...
        if (w != h) throw new IllegalStateException("LableTextView's width must equal to height");
    ...
    public LableTextView setText(String str) {
        mSlantedText = str;
        postInvalidate();
        return this;
    }

    public LableTextView setText(int res) {
    ...
    public LableTextView setSlantedBackgroundColor(@ColorRes int color) {
        mSlantedBackgroundColor = BaseApplication.instance.getResources().getColor(color);
        mPaint.setColor(mSlantedBackgroundColor);
        postInvalidate();
        return this;
    }

    public LableTextView setTextColor(int color) {
        mTextColor = color;
        mTextPaint.setColor(mTextColor);
        postInvalidate();
        return this;
    ...
    /**
     * @param mode :
     *             LableTextView.MODE_LEFT : top left
     *             LableTextView.MODE_RIGHT :top right
     * @return this
     */
    public LableTextView setMode(int mode) {
    ...
    public LableTextView setTextSize(int size) {
        this.mTextSize = size;
        mTextPaint.setTextSize(mTextSize);
        postInvalidate();
        return this;
     ...
     * @return this
     */
    public LableTextView setSlantedLength(int length) {
