...
public class HeaderPositionCalculator {
    ...
    private final LinearLayoutOrientationUtils mOrientationProvider;
    ...
    public HeaderPositionCalculator(StickyRecyclerHeadersAdapter adapter) {
        mAdapter = adapter;
        mOrientationProvider = new LinearLayoutOrientationUtils();
    }
    ...
    public boolean hasStickyHeader(View itemView, int orientation, int position) {
        int offset, margin;
        mOrientationProvider.initMargins(mTempRect1, itemView);
    ...
    public void initHeaderBounds(Rect bounds, RecyclerView recyclerView, View header, View firstView, boolean firstHeader) {
        ...
        if (firstHeader && isStickyHeaderBeingPushedOffscreen(recyclerView, header)) {
            ...
            View secondHeader = getHeader(recyclerView, firstViewUnderHeaderPosition);
    ...
    private void initDefaultHeaderOffset(Rect headerMargins, RecyclerView recyclerView, View header, View firstView, int orientation) {
        ...
        mOrientationProvider.initMargins(mTempRect1, header);
    ...
    private boolean isStickyHeaderBeingPushedOffscreen(RecyclerView recyclerView, View stickyHeader) {
        ...
        if (firstViewUnderHeaderPosition > 0 && hasNewHeader(firstViewUnderHeaderPosition, isReverseLayout)) {
            View nextHeader = getHeader(recyclerView, firstViewUnderHeaderPosition);
            mOrientationProvider.initMargins(mTempRect1, nextHeader);
            mOrientationProvider.initMargins(mTempRect2, stickyHeader);
    ...
    private void translateHeaderWithNextHeader(RecyclerView recyclerView, int orientation, Rect translation,
        ...
        mOrientationProvider.initMargins(mTempRect1, nextHeader);
        mOrientationProvider.initMargins(mTempRect2, currentHeader);
        if (orientation == LinearLayoutManager.VERTICAL) {
    ...
    private boolean itemIsObscuredByHeader(RecyclerView parent, View item, View header, int orientation) {
        ...
        if (adapterPosition == RecyclerView.NO_POSITION || getHeader(parent, adapterPosition) != header) {
