package com.Allicnce.taobaoAlliance.common.tick;
...
class BaseCountdown {
    ...
    public void initStyleAttr(Context context, TypedArray ta) {
        ...
        mTimeTextSize = ta.getDimension(R.styleable.CountdownView_timeTextSize, DisplayUtil.sp2px(12));
        ...
        mSuffixTextSize = ta.getDimension(R.styleable.CountdownView_suffixTextSize, DisplayUtil.sp2px(12));
...
//        if (!isShowMinute && !isShowSecond) isShowSecond = true;
    ...
    private void initSuffixMargin() {
        int defSuffixLRMargin = DisplayUtil.dp2px(DEFAULT_SUFFIX_LR_MARGIN);
    ...
    private float getDayAndHourContentWidth() {
        ...
        if (isShowHour) {
            String tempHour = formatNum(mHour);
    ...
    public void setTimeTextSize(float textSize) {
        if (textSize > 0) {
            mTimeTextSize = DisplayUtil.sp2px(textSize);
            mTimeTextPaint.setTextSize(mTimeTextSize);
        }
    }
    ...
    public void setSuffixTextSize(float textSize) {
        if (textSize > 0) {
            mSuffixTextSize = DisplayUtil.sp2px(textSize);
            mSuffixTextPaint.setTextSize(mSuffixTextSize);
        }
    }
    ...
    public void setSuffixLRMargin(float suffixLRMargin) {
        mSuffixLRMargin = DisplayUtil.dp2px(suffixLRMargin);
    ...
    public boolean setSuffixMargin(Float suffixDayMarginL, Float suffixDayMarginR,
        ...
        if (null != suffixDayMarginL) {
            mSuffixDayLeftMargin = DisplayUtil.dp2px(suffixDayMarginL);
            isRef = true;
        }
        if (null != suffixDayMarginR) {
            mSuffixDayRightMargin = DisplayUtil.dp2px(suffixDayMarginR);
            isRef = true;
        }

        if (null != suffixHourMarginL) {
            mSuffixHourLeftMargin = DisplayUtil.dp2px(suffixHourMarginL);
            isRef = true;
        }
        if (null != suffixHourMarginR) {
            mSuffixHourRightMargin = DisplayUtil.dp2px(suffixHourMarginR);
            isRef = true;
        }

        if (null != suffixMinuteMarginL) {
            mSuffixMinuteLeftMargin = DisplayUtil.dp2px(suffixMinuteMarginL);
            isRef = true;
        }
        if (null != suffixMinuteMarginR) {
            mSuffixMinuteRightMargin = DisplayUtil.dp2px(suffixMinuteMarginR);
            isRef = true;
        }

        if (null != suffixSecondMarginL) {
            mSuffixSecondLeftMargin = DisplayUtil.dp2px(suffixSecondMarginL);
            isRef = true;
        }
        if (null != suffixSecondMarginR) {
            mSuffixSecondRightMargin = DisplayUtil.dp2px(suffixSecondMarginR);
            isRef = true;
        }

        if (null != suffixMillisecondMarginL) {
            mSuffixMillisecondLeftMargin = DisplayUtil.dp2px(suffixMillisecondMarginL);
            isRef = true;
        }
